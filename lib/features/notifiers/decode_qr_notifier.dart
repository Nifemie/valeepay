...
class DecodeQrNotifier extends StateNotifier<DataState<DecodeQrResponse>> {
  ...
  DecodeQrNotifier(this._repository) : super(DataState<DecodeQrResponse>.initial());
  ...
  Future<void> decodeFromBase64(String base64Data) async {
    ...
    try {
      ...
      final parsed = await _repository.decodeQr('data:image/png;base64,$base64Data');
      state = state.copyWith(isInitialLoading: false, isDataAvailable: true, data: [parsed], message: 'Success');
    } catch (e, st) {
...
final decodeQrNotifierProvider = StateNotifierProvider<DecodeQrNotifier, DataState<DecodeQrResponse>>((ref) {
